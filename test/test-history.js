...
describe("history", () => {
  ...
  it("allows changes that aren't part of the history", () => {
    let state = mkState()
    state.type("hello")
    state.apply(state.tr.insertText("oops", 1).setMeta("addToHistory", false))
    state.apply(state.tr.insertText("!", 10).setMeta("addToHistory", false))
    state.command(undo)
  ...
  function unsyncedComplex(state, doCompress) {
    state.type("hello")
    state.apply(closeHistory(state.tr))
    state.type("!")
    state.apply(state.tr.insertText("....", 1).setMeta("addToHistory", false))
    state.apply(state.tr.split(3))
    ist(state.doc, doc(p(".."), p("..hello!")), eq)
    state.apply(state.tr.split(2).setMeta("addToHistory", false))
  ...
  it("supports overlapping edits that aren't collapsed", () => {
    let state = mkState()
    state.apply(state.tr.insertText("h", 1).setMeta("addToHistory", false))
    state.type("ello")
    state.apply(closeHistory(state.tr))
    state.apply(state.tr.delete(1, 6))
    ist(state.doc, doc(p()), eq)
  ...
  it("supports overlapping unsynced deletes", () => {
    ...
    state.apply(closeHistory(state.tr))
    state.type("hello")
    state.apply(state.tr.delete(1, 8).setMeta("addToHistory", false))
    ist(state.doc, doc(p()), eq)
    state.command(undo)
    ist(state.doc, doc(p()), eq)
  })
  ...
  it("supports non-tracked changes next to tracked changes", () => {
    ...
    state.type("o")
    state.apply(state.tr.split(1))
    state.apply(state.tr.insertText("zzz", 4).setMeta("addToHistory", false))
    state.command(undo)
  ...
  it("can go back and forth through history when preserving items", () => {
    ...
    state.type(" two")
    state.apply(closeHistory(state.tr))
    state.apply(state.tr.insertText("xxx", state.selection.head).setMeta("addToHistory", false))
    state.type(" three")
    state.apply(state.tr.insertText("zero ", 1))
    state.apply(closeHistory(state.tr))
    state.apply(state.tr.split(1))
    state.textSel(1)
    state.type("top")
    state.apply(state.tr.insertText("yyy", 1).setMeta("addToHistory", false))
    for (let i = 0; i < 3; i++) {
  ...
  it("rebases selection on undo", () => {
    ...
    state.apply(state.tr.insert(1, schema.text("---")).setMeta("addToHistory", false))
    state.command(undo)
